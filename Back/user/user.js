        ...
        if (results.length > 0) {
          ...
          bcrypt.compare(password, user.password, (err, isMatch) => {
            if (err) {
              ...
              res
                .status(200)
                .json({ token, message: "Vous allez être redirigé" });
            } else {
              res
                .status(401)
  ...
  connection.query(
    ...
    (error, results) => {
      if (error) {
        ...
        res.status(200).json({
          message: "Vous avez été Inscrit, Vous allez etre redirigez",
        });
      }
    }
  );
...
app.get("/", (req, res) => {
  connection.query("SELECT username FROM user", (error, results) => {
    if (error) {
      res
        .status(500)
