...
router.post("/record", async (req, res) => {
  try {
    ...
    let probaEquipe1 = 1 / (1 + Math.pow(10, (MMRMoyenEquipe2 - MMRMoyenEquipe1) / 2000));
    let probaEquipe2 = 1 / (1 + Math.pow(10, (MMRMoyenEquipe1 - MMRMoyenEquipe2) / 2000));
    ...
    equipe1Joueurs.forEach(joueur => {
        ...
        const MMRafter = MMRactuel + (coeffMMR * 2) * ((winner === equipe1 ? 1 : 0) - probaEquipe1) * (1 + (Math.abs(scoreEquipe1 - scoreEquipe2) * 0.05))
    ...
    equipe2Joueurs.forEach(joueur => {
        const MMRactuel = joueur.MMR;
        const MMRafter = MMRactuel + (coeffMMR * 2) * ((winner === equipe2 ? 1 : 0) - probaEquipe2) * (1 + (Math.abs(scoreEquipe2 - scoreEquipe1) * 0.05))
